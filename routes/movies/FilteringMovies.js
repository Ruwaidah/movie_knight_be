...
router.post("/", (req, res) => {
  ...
  let results;
  let movieFilter = [];
  let dateToFilter = req.body.days;
        ...
        .then((response) => {
          movieById(req.body.movies[i], req, 3, 4).then((response2) => {
            if (response.data.length == 0 && response2.data.length == 0) {
              ...
              movieFilter = filtering(
                response2.data[0].showtimes,
                timeFormat,
                dateToFilter
              );
              ...
              id = response2.data[0].tmsId;
              movieFilter = filtering(
                [...response.data[0].showtimes, ...response2.data[0].showtimes],
                timeFormat,
                dateToFilter
              );
              results = filteDuplicate(movieFilter);
            }
            moviesAfterFilter.push({
              id: id,
              movie: title,
              showtimes: results,
            });
...
function movieById(id, req, days, number) {
  return axios.get(
    `http://data.tmsapi.com/v1.1/movies/${id}/showings?startDate=${getday(
      days
    )}&zip=${checkZip(req)}&radius=10&numDays=${number}&api_key=${
      process.env.API_KEY
    }`
  );
}
...
function filtering(data, times, dates) {
  let showTimefilterDate = data.filter((date) =>
    dates.includes(date.dateTime.split("T")[0])
  );
  showTimefilterDate = showTimefilterDate.filter((time) =>
...
function filteDuplicate(movies) {
  ...
  let dates = removeDuplicateUsingSet(
    movies.map((date) => date.dateTime.split("T")[0])
  );
  theatersName.map((theater) => {
    let showTimes = [];
    let dataForOneTheater = movies.filter(
      (movie) => movie.theatre.name == theater
    );
    for (let i = 0; i < dates.length; i++) {
      var dateStr = dates[i].replace("-", "/");
      var dayName = getDayName(dateStr, "en-US");
      times = dataForOneTheater.filter(
        (info) => info.dateTime.split("T")[0] == dates[i]
      );
